...
void crearJugador(tJugador *j, const char *nombre, const char *usuario,
                  int posicionInicial, int vidas) {
...
int cargarConfiguracion(const char *archivo, tConfiguracion *cfg) {
  ...
  leerArchivoTxt(arch, cfg, sizeof(tConfiguracion), sizeof(char) * 80,
                 trozarConfig);
  fclose(arch);
...
void trozarConfig(void *config, const void *dato) {
  ...
  if (claveIgual(parametro, "TOTAL_CASILLAS") ||
      claveIgual(parametro, "cantidad_posiciones")) {
    c->totalCasillas = valor;
  } else if (claveIgual(parametro, "VIDAS_INICIALES") ||
             claveIgual(parametro, "vidas_inicio")) {
    c->vidasIniciales = valor;
  } else if (claveIgual(parametro, "CANTIDAD_BANDIDOS") ||
             claveIgual(parametro, "maximo_bandidos")) {
    c->cantidadBandidos = valor;
  } else if (claveIgual(parametro, "CANTIDAD_PREMIOS") ||
             claveIgual(parametro, "maximo_premios")) {
    c->cantidadPremios = valor;
  } else if (claveIgual(parametro, "CANTIDAD_VIDAS") ||
             claveIgual(parametro, "maximo_vidas_extra")) {
    c->cantidadVidas = valor;
  } else if (claveIgual(parametro, "CANTIDAD_OASIS") ||
             claveIgual(parametro, "maximo_oasis")) {
    c->cantidadOasis = valor;
  } else if (claveIgual(parametro, "CANTIDAD_TORMENTAS") ||
             claveIgual(parametro, "maximo_tormentas")) {
...
int moverJugador(tJuego *juego, tMovimiento movimiento,
  ...
  /* Si el destino se pasa de los limites, se aplica rebote para mantenerlo
   * dentro del tablero. */
...
int aplicarEfectoCasilla(tJugador *j, tCasilla *casilla) {
  ...
  /* PRIORIDAD DE EFECTOS:
     ...
     2. Oasis    → otorga proteccion; tiene prioridad sobre todos los efectos
     negativos.
     3. Tormenta → bloqueada si el jugador obtuvo oasis esta o la vuelta
     anterior.
     4. Vidas / Premios → se recogen siempre (efectos positivos, desaparecen al
     ser tomados). Bandidos: su colision se resuelve en verificarColision, donde
     tambien se respeta la proteccion del oasis. */
  ...
  if (casilla->oasis) {
    j->protegidoOasis = 1;
    obtuvoOasis = 1;
    puts("El jugador descansa en un oasis y estara protegido hasta su proximo "
         "turno.");
  }
  ...
  // El oasis pesa mas: si hay oasis en esta casilla, la tormenta no tiene
  // efecto.
  if (casilla->tormenta) {
    if (j->protegidoOasis && !obtuvoOasis) {
      ...
      puts("El oasis neutraliza la tormenta; la proteccion se mantiene para el "
           "proximo turno.");
    } else {
      puts(
          "El jugador pierde el proximo turno debido a una tormenta de arena.");
      j->perdidoTurno = 1;
    }
  }
...
void inicializarJuego(tJuego *juego, tConfiguracion *cfg) {
  // Se copia la configuracion y se construyen las estructuras necesarias para
  // jugar.
  juego->config = *cfg;
...
int moverBandido(tJuego *juego, tBandido *b, tMovimiento movimiento,
  ...
  // Se actualiza la posicion y se marca la nueva casilla ocupada por el
  // bandido.
  b->posicion = nuevaPos;
...
int verificarColision(tJuego *juego, tBandido *b, tCasilla *casillaActual) {
  // Solo hay choque real si el bandido sigue activo y coincide con la posicion
  // del jugador.
  ...
  // Si no hay proteccion, el bandido queda desactivado y el jugador pierde una
  // vida.
  b->activo = 0;
  if (casillaActual && casillaActual->bandidos > 0) {
    casillaActual->bandidos--;
  ...
  // El jugador vuelve al campamento inicial y se revisan colisiones en esa
  // casilla.
...
int verificarDerrota(const tJugador *j) { return j->vidas <= 0; }
...
void guardarCaravana(const char *archivo, const tJuego *juego) {
  ...
  fprintf(f, "Posiciones: %d | Vidas: %d | Bandidos: %d\n\n",
          juego->config.totalCasillas, juego->config.vidasIniciales,
  ...
  do {
    ...
    if (c->tormenta)
      fprintf(f, "T ");
    if (c->vidas)
      fprintf(f, "%uV ", c->vidas);
    if (c->premios)
      fprintf(f, "%uP ", c->premios);
    if (c->bandidos)
      fprintf(f, "%uB ", c->bandidos);
    if (c->jugador)
      fprintf(f, "J ");
    if (c->normal)
      fprintf(f, ".");
  ...
  fprintf(f, "\nReferencias: J jugador, B bandido, I inicio, S refugio, P "
             "premio, V vida, O oasis, T tormenta, . ruta.\n");
  fclose(f);
}
...
int cargarCaravana(const char *nombreArchivo, tJuego *juego) {
  ...
  // Lee la casilla hasta cargar la cantidad de casillas indicadas por
  // configuracion o hasta el fin del archivo
...
void comprobarColisionesJB(void *bandido, void *contexto, void *extra) {
  ...
  tCasilla *casillaActual =
      buscarCasilla(&juego->tablero, bandidoActual->posicion, cmpPosCasillas);
...
int posicionarJugador(tJuego *juego, int posicion) {
  ...
  casillaActual =
      buscarCasilla(&juego->tablero, juego->jugador.posicion, cmpPosCasillas);
  if (!casillaActual) {
    return 0;
  }
  ...
  // Tras mover al jugador, se revisa si algun bandido quedo en la misma
  // casilla.
...
int IndexarArchivoOrdenado(FILE *archIndx, char *nombreArch,
                           ArbolBin *arbolIndx, unsigned tam) {
  (void)archIndx;
  ...
  cargarElementosOrdenadosArbolBin(arbolIndx, archIndx, 0, cantReg, &tam,
                                   guardarIndiceEnNodo);
...
void actualizarJugadores(FILE *archJug, ArbolBin *indice,
                         const tJugador *nuevo) {
...
void guardNodoIndxEnArchivo(void *dato, size_t tam, unsigned nivel,
                            void *params) {
...
int crearRanking(tLista *ranking, FILE *archPart, ArbolBin *indice,
                 FILE *archJug) {
  (void)indice;
  (void)archJug;
  crearLista(ranking);
...
void cargarNombres(void *dato, void *contexto, void *param) {
  ...
  if (pos < 0 ||
      leerPos(archJug, &auxJug, sizeof(tRegistroJugador), pos) != 1) {
...
void cargarRanking(void *ranking, const void *dato) {
  ...
  insertarEnOrden(ranking, &reg, sizeof(tRegistroRanking), cmpRankUsuarios,
                  sumarRankPuntos);
}
...
int sumarRankPuntos(void **dato1, unsigned *tam1, const void *dato2,
                    unsigned tam2) {
...
int actualizarRegistroPartidas(tArchivos archivos, ArbolBin *indice,
                               tLista *ranking,
                               const tRegistroPartida *partida) {
  ...
  if (pos < 0 ||
      leerPos(archivos.archJug, &auxJug, sizeof(tRegistroJugador), pos) != 1)
    return 0;
  ...
  insertarEnOrden(ranking, &nuevo, sizeof(tRegistroRanking), cmpRankUsuarios,
                  sumarRankPuntos);
...
void mostrarRanking(const void *reg, void *param) {
  ...
  (void)param;
  printf("Usuario:%-15s | Nombre:%-15s | Puntos: %02d | Cantidad de Partidas: "
         "%02d\n",
         ranking->usuario, ranking->nombre, ranking->puntuacionTotal,
...
void mostrarPartidas(void *a, const void *b) {
  ...
  (void)a;
  printf("Usuario:%s | Puntos: %d | Mov: %d\n", p->usuario, p->puntuacion,
         p->cantidadMovimientos);
}
